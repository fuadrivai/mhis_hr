            ...
            <div class="row">
                <div class="col-sm-12">
                    <form id="location-form" autocomplete="OFF">
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input required type="text" id="name" value="{{isset($location)?$location->name:''}}" class="form-control" name="name" />
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <select name="branch_id" id="branch_id" class="form-control">
                                                @foreach ($branches as $branch)
                                                    @if (isset($location))
                                                        <option {{$location->branch_id == $branch->id?'selected':''}} value="{{$branch->id}}">{{$branch->name}}</option>
                                                    @else
                                                        <option {{isset($location)}} value="{{$branch->id}}">{{$branch->name}}</option>
                                                    @endif
                                                @endforeach
                                            </select>
                                        </div>
                                    ...
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            ...
                                            <input type="text" value="{{isset($location)?$location->latitude:''}}" id="latitude" class="form-control" name="latitude" />
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            <label for="longitude">Longitude *</label>
                                            <input type="text" id="longitude" value="{{isset($location)?$location->longitude:''}}" class="form-control" name="longitude" />
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            <label for="radius">Radius *</label>
                                            <input required type="text" id="radius" value="{{isset($location)?$location->radius:''}}" class="form-control" name="radius" />
                                        </div>
                                    </div>
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            <label for="description">Description</label>
                                            <Textarea id="description" rows="4" class="form-control" name="description">{{isset($location)?$location->description:''}}</Textarea>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <button data-toggle="modal" data-target="#right-modal-user" type="button" class="btn btn-success btn-sm text-white btn-add-user"><i class="fa fa-plus"></i> Add User</button>
                            </div>
                        </div>
                        <br>
                        <br>
                        <div class="row">
                            <div class="col-md-12">
                                <table id="tbl-employee-location" class="table table-striped table-bordered table-sm" style="width: 100%">
                                    <thead>
                                        <tr>
                                            <th>Name</th>
                                            <th>Branch</th>
...
<div class="modal modal-right fade" id="right-modal-user" tabindex="-1" role="dialog" aria-labelledby="right_modal_lg">
...
<script>(g=>{
    ...
    let d=b.maps||(b.maps={}),r=new Set,e=new URLSearchParams,u=()=>h||(h=new Promise(async(f,n)=>
        ...
        ({key: "AIzaSyAhPd4p9-KRS06GMC0pzoV_vbd7q5_kGuk", v: "weekly"});
</script>
<script>
    ...
    function getLocationById(){
        ajax({id:dataId}, `{{URL::to('location/show')}}`, "GET",
            function(json) {
                pinLocation = json;
                reloadJsonDataTable(tblUserLocation,pinLocation.employees);
        })
    ...
    function submitLocation(){
        pinLocation.branch = {id:$('#branch_id').val()};
    ...
    async function initMap() {
        ...
        const myLatlng ={ lat: -6.1944, lng: 106.8229 };
