                ...
                <div class="row">
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input value="{{ $search }}" type="text"  name="search" id="search" class="form-control" placeholder="Full name or email" >
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <select name="status" id="status" class="select2 form-control">
                                ...
                                <option {{$query['status']=="permanent"?"selected":""}} value="permanent">Permanent</option>
                                <option {{$query['status']=="contract"?"selected":""}} value="contract">Contract</option>
                                <option {{$query['status']=="freelance"?"selected":""}} value="freelance">Freelance</option>
                            </select>
                        </div>
                    </div>
                    <div class="col-md-3">
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="row pl-2 pr-2 d-flex">
                    ...
                    <i class="pl-2 pt-2"><b>Show {{$page['from']}} to {{$page['to']}} of total {{$page['total']}}</b></i>
                </div>
            </div>
            <div class="col-md-6">
                <div class="row">
                    <div class="col-md-12 text-right">
                        <div class="btn-group" role="group">
                            <a data-toggle="modal" data-target="#importExcel" href="/employee/create" class="btn btn-info text-white btn-add-employee"><i class="fa fa-upload"></i> Import</a>
                            <a href="/employee/create" class="btn btn-success text-white btn-add-employee"><i class="fa fa-plus"></i> Add User</a>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="row">
                    <div class="col col-xs-12">
                        @if ((!isset($emp->personal->avatar))|| ($emp->personal->avatar==""))
                            <img width="80" src="{{ asset('images/user.png') }}" class="rounded float-left img-thumbnail" alt="{{ $emp->personal->fullname }}">
                        @else
                            <img width="80" src="{{ asset('storage/'.$pr->image) }}" class="rounded float-left img-thumbnail" alt="{{ $emp->personal->fullname}}">
                        @endif
                    </div>
                    <div class="col-3 col-xs-12 text-left">
                        <label class="m-0 p-0 font-weight-bold" for="">{{ Str::upper($emp->personal->fullname) }}</label>
                        <p class="m-0 p-0 font-weight-bold" for=""><i class="fa fa-envelope text-info"></i> {{ $emp->personal->email }}</p>
                        <p class="m-0 p-0 font-weight-bold" for=""><i class="fa fa-phone text-info"></i> {{ $emp->personal->mobile_phone }}</p>
                        <p class="m-0 p-0">NIK : <label class="m-0 p-0 font-weight-bold" for="">{{ $emp->employment->employee_id??"" }}</label></p>
                    ...
                    <div class="col-2 text-center">
                        <div class="row">
                            <div class="col-12">
                                ...
                                <p  class="m-0 p-0">
                                    <label for="">{{ $emp->employment->join_date==""?"--": \Carbon\Carbon::parse($emp->employment->join_date)->format('d F Y')}}</label>
                                </p>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-12">
                                ...
                                <p  class="m-0 p-0">
                                    <label for="">{{ $emp->employment->end_date!=""? \Carbon\Carbon::parse( $emp->employment->end_date)->format('d F Y'):"--"}}</label>
                                </p>
                            </div>
                        </div>
                    </div>
                        ...
                        <p  class="m-0 p-0">
                            @switch($emp->employment->employment_status)
                                ...
                                @case("freelance")
                                    ...
                                    @break
                                @default
                                    <span class="badge badge-secondary">{{$emp->employment->employment_status??"--"}}</span>
                            @endswitch
                        </p>
                    </div>
                    <div class="col text-center">
    ...
    <div class="row">
        <div class="col-md-6">
            <div class="row pl-2 pr-2 d-flex">
                ...
                <i class="pt-1"><b> Show {{$page['from']}} to {{$page['to']}} of total {{$page['total']}}</b></i>
            </div>
        </div>
        <div class="col-md-6">
