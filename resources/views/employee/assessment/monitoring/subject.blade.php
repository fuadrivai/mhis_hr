    ...
    <div class="col-md-12 col-sm-12">
        <div class="x_panel">
            <div class="x_title">
                ...
                <a href="{{ route('employee.assessment.monitoring.show', $target->id) }}" class="btn btn-default btn-sm pull-right"><i class="fa fa-arrow-left"></i> Back to Subject List</a>
                                            ...
                                            <div style="font-size: 12px; margin-top: 15px;">
                                                ...
                                                <table class="table table-bordered table-condensed" style="background-color: #fff; margin-top: 5px; font-size: 11px;">
                                                    ...
                                                    <tbody>
                                                        <tr>
                                                            <td class="align-middle">
                                                                <a href="{{ $detail['submission']->file_link }}" target="_blank"><i class="fa fa-file-pdf-o"></i> View File</a>
                                                            </td>
                                                            <td class="align-middle">
                                                                <span class="label {{ $detail['submission']->status == 'approved' ? 'label-success' : ($detail['submission']->status == 'need_revision' ? 'label-warning' : 'label-primary') }}">{{ ucfirst(str_replace('_', ' ', $detail['submission']->status)) }}</span>
                                                            </td>
                                                            <td class="align-middle">
                                                                @if($detail['submission']->approvals->isNotEmpty())
                                                                    {{ $detail['submission']->approvals->last()->approverEmployee->user->name ?? 'Unknown' }}
                                                                @else
