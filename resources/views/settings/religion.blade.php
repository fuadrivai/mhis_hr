    ...
    <div class="x_panel">
        <div class="x_title">
            <ul class="nav navbar-right panel_toolbox">
                <li>
                    <button data-toggle="modal" data-target="#modal-data" type="button" class="btn btn-success btn-sm text-white btn-add"><i class="fa fa-plus"></i> Add Religion</button>
                </li>
            </ul>
...
<script>
    $(document).ready(function(){
        ...
        $("#tbl-datatable").on('click','.btn-data',function(){
            ...
            $('#form-data').attr('action',`/religion/${id}`)
        });

        $('.btn-add').on('click',function(){
            $('#form-method').append(`@method('post')`);
            $('#form-data').attr('action',"/religion")
        });
    })
