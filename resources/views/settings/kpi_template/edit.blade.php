...
@section('content-child')
    <div class="col-md-12 col-sm-12">
        <div class="x_panel">
            <div class="x_title">
                <h2>Edit KPI Template</h2>
                <div class="clearfix"></div>
            </div>
            <div class="x_content">
                <form action="{{ route('kpi-template.update', $kpi_template->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="form-group">
                        <label>Template Name</label>
                        <input type="text" name="name" class="form-control" value="{{ $kpi_template->name }}" required>
                    </div>
                    <div class="form-group">
                        <label>
                            <input type="checkbox" name="is_public" value="1" {{ $kpi_template->is_public ? 'checked' : '' }}> Make this template Public (Visible to other users)
                        </label>
                    </div>
                    ...
                    <div id="managerial-container">
                        ...
                        @foreach($kpi_template->targets->where('type', 'managerial') as $mTarget)
                            <div class="card bg-light p-3 mb-2" id="mTarget-{{ $mIndex }}">
                                <div class="row">
                                    <div class="col-md-7">
                                        <input type="text" name="managerial_targets[{{ $mIndex }}][name]" class="form-control" value="{{ $mTarget->name }}" required>
                                    </div>
                                    <div class="col-md-3">
                                        <input type="number" step="0.01" name="managerial_targets[{{ $mIndex }}][weight]" class="form-control" value="{{ $mTarget->weight }}">
                                    </div>
                                    ...
                                    <div id="mSubContainer-{{ $mIndex }}">
                                        @foreach($mTarget->subTargets as $sub)
                                            ...
                                            <div class="row mb-1" id="mSub-{{ $subIndex }}">
                                                <div class="col-md-5">
                                                    <input type="text" name="managerial_targets[{{ $mIndex }}][sub_targets][{{ $subIndex }}][name]" class="form-control form-control-sm" value="{{ $sub->name }}" required>
                                                </div>
                                                <div class="col-md-3">
                                                    <input type="number" step="0.01" name="managerial_targets[{{ $mIndex }}][sub_targets][{{ $subIndex }}][target_score]" class="form-control form-control-sm" value="{{ $sub->target_score }}" required>
                                                </div>
                                                <div class="col-md-2">
                                                    <input type="number" step="0.01" name="managerial_targets[{{ $mIndex }}][sub_targets][{{ $subIndex }}][weight]" class="form-control form-control-sm" value="{{ $sub->weight }}">
                                                </div>
                                                <div class="col-md-2">
                    ...
                    <div id="tal-container">
                        ...
                        @foreach($kpi_template->targets->where('type', 'tal') as $tTarget)
                            <div class="row mb-2" id="tTarget-{{ $tIndex }}">
                                <div class="col-md-5">
                                    <input type="text" name="tal_targets[{{ $tIndex }}][name]" class="form-control" value="{{ $tTarget->name }}" required>
                                </div>
                                <div class="col-md-3">
                                    <input type="number" step="0.01" name="tal_targets[{{ $tIndex }}][target_score]" class="form-control" value="{{ $tTarget->target_score }}" required>
                                </div>
                                <div class="col-md-2">
                                    <input type="number" step="0.01" name="tal_targets[{{ $tIndex }}][weight]" class="form-control" value="{{ $tTarget->weight }}" required>
                                </div>
                                <div class="col-md-2 text-right">
                    ...
                    <hr>
                    <button type="submit" class="btn btn-primary"><i class="fa fa-save"></i> Update Template</button>
